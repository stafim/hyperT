...
Set up a **`mockup/` folder** inside any React project as an isolated frontend sandbox for rapid UI prototyping. Components are rendered in isolation via a vite dev server, and each component gets a `/preview/ComponentName` route that can be embedded as an iframe shape on the workspace canvas.
...
1. A vite dev server runs in a `mockup/` folder, separate from the main app
2. A custom Vite plugin (`mockupPreviewPlugin`) uses `fast-glob` and file watching to discover components in `mockup/src/components/mockups/`
...
Run the setup script to copy the template scaffold and install dependencies. The script outputs the **dev URL** you'll need for embedding iframes:

```bash
bash .local/skills/mockup-sandbox/setup.sh
```

The output will include:

```text
=== Mockup Sandbox Ready ===
Dev URL: https://<your-repl-domain>:8000
Preview URL pattern: https://<your-repl-domain>:8000/preview/{folder}/{ComponentName}
```

**Save the Dev URL from the output** -- use it when creating iframe shapes on the canvas. Do not guess or template the URL.
...
await configureWorkflow({
    name: "Mockup Sandbox",
    command: "cd mockup && pnpm install && pnpm run dev",
    waitForPort: 8000,
    outputType: "webview",
...
**Important:** Use `pnpm`. The workflow command includes `pnpm install` so dependencies are installed on every start, including restarts.

### Step 3: Create mockup components

**Verify the component directory first.** Before creating any files, list `mockup/` to confirm that `src/components/mockups/` exists. Use the verified path for all component creation and subagent delegation.

Create components in `mockup/src/components/mockups/`:

```tsx
// mockup/src/components/mockups/pricing-cards/Minimal.tsx
...
### Step 4: Embed on the canvas

Create `iframe` shapes on the workspace canvas.
Use the **Dev URL from the setup script output**. Do not use template strings -- use the actual URL.
...
```json
{
  ...
  "shape": {
    ...
    "url": "https://<your-domain>.replit.dev:8000/preview/pricing-cards/Minimal",
    "componentPath": "mockup/src/components/mockups/pricing-cards/Minimal.tsx",
    "componentName": "Minimal Pricing Card"
  }
}
```
If you need the URL again later, run: `echo "https://${REPLIT_DOMAINS}:8000"`

### Step 5: Layout and focus
...
Place images in `mockup/public/images/` and reference by URL path:
...
Place images in `mockup/src/assets/` and import them:
...
The `@` alias maps to `mockup/src/`, so `@/assets/hero.png` resolves to `mockup/src/assets/hero.png`.
...
await generateImage({
    images: [{
        ...
        outputPath: "mockup/public/images/headphones.png",
        aspectRatio: "1:1"
    }]
});
```

Then reference: `<img src="/images/headphones.png" />`.

**Path warning:** The `outputPath` must start with `mockup/public/` -- NOT just `public/`. The main app's public folder is not served by the mockup dev server. Using `outputPath: "public/images/hero.png"` (without the `mockup/` prefix) will 404 in mockup iframes.
...
**Custom fonts.** For fonts outside the bundled set, add a non-blocking `<link>` tag to `mockup/index.html`. Do not use CSS `@import url(...)` — it is render-blocking and will delay all Tailwind styles until the font finishes downloading.

```html
<!-- in mockup/index.html <head> -->
...
1. Edit `mockup/package.json` directly and add the dependency
2. Restart the "Mockup Sandbox" workflow -- `pnpm install` runs as part of the workflow command
...
```json
{
  ...
  "shapeId": "pricing-bold",
  "updates": {
    "url": "https://<dev-url>:8000/preview/pricing-cards/Bold",
    "componentPath": "mockup/src/components/mockups/pricing-cards/Bold.tsx",
    "state": "live"
  }
}
```
...
```text
Create a mockup component at mockup/src/components/mockups/pricing-cards/Bold.tsx
...
```text
Create a mockup page at mockup/src/components/mockups/crm-dashboard/Dashboard.tsx
...
When done, update the canvas iframe to show the real preview:
  ...
  URL: https://<dev-url>:8000/preview/crm-dashboard/Dashboard
  componentPath: mockup/src/components/mockups/crm-dashboard/Dashboard.tsx
  state: "live"
```
...
4. **Verify paths before delegating.** Before passing file paths to subagents, list `mockup/` to confirm `src/components/mockups/` exists and pass the verified full path. Getting this wrong means files land in a directory the Vite plugin never scans.

5. **Tell subagents the image path convention.** Always include this in the subagent task: "Place all images in `mockup/public/images/` and reference them as `<img src="/images/filename.jpg" />`. Do NOT put images in `src/assets/` and reference them by URL path — Vite does not serve `src/` as static assets and they will 404. For `generateImage`, use `outputPath` starting with `mockup/public/images/`."
...
When extracting existing components, create `_group.css` in the extraction's group folder with the main app's `:root` and `.dark` CSS variable blocks plus any font `@import`s the app uses. Each extracted component imports `./_group.css` explicitly. Do not edit the global `mockup/src/index.css` — that would leak one app's tokens into every unrelated mockup group in the sandbox. See the {{skill("mockup-extract")}} skill for the full process.
...
2. Verify the missing file exists under `mockup/src/` (not the main app).
